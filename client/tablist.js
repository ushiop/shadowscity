...
API.onUpdate.connect(function () {
    // MultiplayerInfo
    if (!API.isChatOpen() && API.isControlJustPressed(20 /* MultiplayerInfo */)&&API.getEntitySyncedData(API.getLocalPlayer(),"SC_Login_Status")==1) {
        g_currentState++;
        ...
        if (g_currentState == 3) {
			...
			API.setEntitySyncedData(API.getLocalPlayer(),"SC_RADAR_BIGMAP_ENABLED",true);
        }
        if (g_currentState == 4) {
            g_currentState = 0;
            API.callNative("_SET_RADAR_BIGMAP_ENABLED", false, false);
			
			API.setEntitySyncedData(API.getLocalPlayer(),"SC_RADAR_BIGMAP_ENABLED",false);
        }
    }
    ...
    if (g_currentState == 1 || g_currentState == 2) {
        // Get list data
        ...
        // Headers
        if (g_currentState == 1||g_currentState == 2) {
            API.drawText("昵稱", listX + listPadding, listY - listPadding / 2, 0.4, 255, 255, 0, 255, 0, 0, true, true, 0);
        }
 
        API.drawText("延遲", listX + listWidth - listPadding, listY - listPadding / 2, 0.4, 255, 255, 0, 255, 0, 2, true, true, 0);
        API.drawText("RANK", listX + listWidth - listPadding - 200, listY - listPadding / 2, 0.4, 255, 255, 0, 255, 0, 2, true, true, 0);
        ...
        for (var i = 0; i < listPageCount; i++) {
            ...
            if (g_currentState == 1||g_currentState == 2) {
                API.drawText(player.name, listX + listPadding, listY + listPadding / 2 + listLine * (i + 1), 0.4, color[0], color[1], color[2], 255, 0, 0, true, true, 0);
            }
			API.drawText("" + player.rank, listX + listWidth - listPadding - 200, listY + listPadding / 2 + listLine * (i + 1), 0.4, 100, 100, 100, 255, 0, 2, true, true, 0);
    
            API.drawText("" + player.ping, listX + listWidth - listPadding, listY + listPadding / 2 + listLine * (i + 1), 0.4, 100, 100, 100, 255, 0, 2, true, true, 0);
        }
    }
});
